...
################################## Bring in the thermistor data #############################

#To obtain datafile set up a path
path.hourlysave<-'~/Documents/R/MN_lakes/Timeseries/CombinedData.txt'

#load in the time-series data using the path you just created above
wtr.hourlytemp=load.ts(path.hourlysave)

#to create the color/gradiant map
wtr.heat.map(wtr.hourlytemp)

#to constrain the z column - in this case to make our temperature key have a range from 0 to 30
...
wnd <- read_csv("~/Documents/R/MN_lakes/wind/glff_data_20221013033928.csv", 
...
bth <- read_table("~/Documents/R/MN_lakes/Schmidt_stability/deming_1m.csv")
...
#export df to csv
write.csv(uStar,"~/Documents/R/MN_lakes/Timeseries/Deming_uStar.csv", row.names = FALSE)

#export df to csv
write.csv(lake_num,"~/Documents/R/MN_lakes/Timeseries/Deming_lake_num.csv", row.names = FALSE)
...
#export df to csv
write.csv(internal_energy,"~/Documents/R/MN_lakes/Timeseries/Deming_internal_energy.csv", row.names = FALSE)

################################ Sparkling Lake lake number #################################
